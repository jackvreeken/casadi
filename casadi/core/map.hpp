...
namespace casadi {
  ...
  */
  ...
  /** A map Evaluate in parallel using pthreads
      Note: Do not use this class with much more than the intended number of
      threads for the parallel evaluation as it will cause excessive memory use.
      ...
      \date 2018
  */
  class CASADI_EXPORT MapThreads : public Map {
    friend class Map;
  protected:
    // Constructor (protected, use create function in Map)
    MapThreads(const std::string& name, const Function& f, casadi_int n) : Map(name, f, n) {}
    ...
    /// Type of parallellization
    std::string parallelization() const override { return "threads"; }
